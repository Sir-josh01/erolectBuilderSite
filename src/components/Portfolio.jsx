...
const projects = [
  { id: 1, 
    ...
    img: '../../public/pool-work-1.jpg',
    videoUrl: '../../public/VID-20260312-WA0017.mp4',
    location: 'Maitama, Abuja',
    problem: 'The client had a sloped backyard that was unusable for traditional construction.',
    solution: 'Engineered a cantilevered infinity pool that utilizes the slope for a dramatic overflow effect.',
  ...
  {id: 2, 
    ...
    solution: 'Used accelerated curing concrete and 24-hour engineering shifts.' },
            ...
            {['All', 'Pools', 'Construction'].map((cat) => (
              <button
                ...
                className={`px-6 py-2 rounded-full font-bold transition-all cursor-pointer ${
                  filter === cat ? 'bg-brand-primary text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300'
                }`}
              >
                {cat}
              </button>
          ...
          {filteredProjects.map((project) => (
            ...
            >
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-8 translate-y-8 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-500">
                <span className="text-brand-primary text-xs font-bold uppercase tracking-widest mb-2">{project.category}</span>
                <h4 className="text-white text-2xl font-bold">{project.title}</h4>
                <button className="mt-4 text-white text-sm font-bold border-b-2 border-brand-primary w-fit">VIEW PROJECT</button>
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="relative bg-white dark:bg-gray-800 w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-sm flex flex-col md:flex-row shadow-2xl">
             ...
             <div className="md:w-1/2 h-64 md:h-auto bg-black flex items-center">
              ...
              <div className="md:w-1/2 p-8 md:p-12 space-y-6">
                <div>
                  <span className="text-brand-primary font-bold uppercase tracking-widest text-xs">{selectedProject.category}</span>
                  <h2 className="text-3xl font-bold text-gray-900 dark:text-white mt-2">{selectedProject.title}</h2>
                  <p className="text-gray-500 dark:text-gray-400 flex items-center gap-2 mt-1">
                    <span className="text-brand-primary">📍</span> {selectedProject.location || 'Location Confidential'}
                  </p>
                </div>

                <div className="space-y-4">
                  <div>
                    <h5 className="text-sm font-bold uppercase text-gray-900 dark:text-white border-l-2 border-brand-primary pl-2">The Problem</h5>
                    <p className="text-gray-600 dark:text-gray-400 text-sm mt-2">{selectedProject.problem}</p>
                  </div>
                  <div>
                    <h5 className="text-sm font-bold uppercase text-gray-900 dark:text-white border-l-2 border-brand-primary pl-2">The Solution</h5>
                    <p className="text-gray-600 dark:text-gray-400 text-sm mt-2">{selectedProject.solution}</p>
                  </div>
                </div>
